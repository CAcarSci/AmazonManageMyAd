...
def retrieve(category_id: str, query: str, top_k: int=12) -> List[Tuple[str, dict, float]]:
    vec = embed(query)
...
def generate_answer(category_id: str, question: str, contexts: List[Tuple[str, dict, float]]):
    ctx = "\n\n".join([f"[Score {round(s,3)}] {c}\nMETA: {json.dumps(m)[:400]}" for c,m,s in contexts])
    ...
    r = requests.post(f"{OLLAMA}/api/generate", json={"model": GEN_MODEL, "prompt": prompt, "stream": False}, timeout=120)
    r.raise_for_status()
