...
def fetch_daily(day: str):
    ...
    r = requests.post(
        f"{AMZ_BASE}/reports/{REPORTS_VER}/reports", headers=headers(), json=body
    )
    r.raise_for_status()
    rid = r.json()["reportId"]
    for _ in range(60):
        s = requests.get(
            f"{AMZ_BASE}/reports/{REPORTS_VER}/reports/{rid}", headers=headers()
        ).json()
        ...
        with conn.cursor() as cur:
            for r in rows:
                et = (
                    "keyword"
                    if r.get("keywordId")
                    else ("adGroup" if r.get("adGroupId") else "campaign")
                )
