    ...
    for et, eid, clicks, cost, sales, acos in rows:
        ...
        if sales is None or sales == 0:
            if cost and cost > MIN_SPEND:
                out.append({"entity_type": et, "entity_id": eid, "action": "decrease", "percent": 0.15, "reason": "No sales 14d"})
            continue
        if acos and acos > TARGET_ACOS:
            delta = max(0.10, min(0.25, (acos/TARGET_ACOS - 1)/2))
            out.append({"entity_type": et, "entity_id": eid, "action": "decrease", "percent": round(delta,2), "reason": f"High ACOS {acos:.2f}"})
        elif acos and acos < TARGET_ACOS*0.7 and sales>0:
            out.append({"entity_type": et, "entity_id": eid, "action": "increase", "percent": 0.10, "reason": f"Efficient {acos:.2f}"})
    return out
